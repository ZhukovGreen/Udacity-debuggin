...
def trace_fetch_state(frame, event, arg):
    ...
    global the_state
    global previous_line_num
    global current_iteration

    # YOUR CODE HERE
    # print event, previous_line_num, frame.f_lineno, current_iteration
    if event == 'call':
        previous_line_num = 0
        current_iteration = 1
    if previous_line_num > frame.f_lineno:
        current_iteration += 1
    previous_line_num = frame.f_lineno
    # print '{} == {} and {} == {}'.format(current_iteration,the_iteration,the_line,frame.f_lineno)
    if current_iteration == the_iteration and the_line == frame.f_lineno:
        the_state = copy.deepcopy(frame.f_locals)
...
if result1 == correct1 and result2 == correct2:
    ...
    print False, "\n", "Your output is incorrect for given values. Please read the supplied comments."
